...
namespace day12
{
  class Program
  {
    static void Main(string[] args)
    {
      ...
      var xAxis = GetStepsForRepeat(input, x => x.Velocity.X);
      var yAxis = GetStepsForRepeat(input, x => x.Velocity.Y);
      var zAxis = GetStepsForRepeat(input, x => x.Velocity.Z);

      Console.Out.WriteLine(xAxis * yAxis * zAxis);
    }

    private static BigInteger GetStepsForRepeat(string[] input, Func<Moon, int> getAxis)
    {
      ...
      do
      {
        ...
        CalcNewPosition(moons);
        step++;
      } while (moons.Select(x => Math.Abs(getAxis(x))).Sum() != 0);

      return step;
    }

    private static List<Moon> ReadInput(string[] input)
    {
